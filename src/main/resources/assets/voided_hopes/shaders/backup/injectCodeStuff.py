from pathlib import Path

def inject_uniform_to_vsh(shader_dir: Path, uniform_line: str, end_line: str):
    """
    Inject a uniform declaration at the first line and code before the final }
    in all .vsh files.

    Args:
        shader_dir: Directory containing .vsh files
        uniform_line: Line to add at the beginning (e.g., uniform declaration)
        end_line: Line to add before the final closing brace
    """
    vsh_files = list(shader_dir.glob('*.vsh'))

    if not vsh_files:
        print("No .vsh files found!")
        return

    print(f"Processing {len(vsh_files)} .vsh files...")
    print()

    updated_count = 0

    for vsh_file in vsh_files:
        try:
            with open(vsh_file, 'r', encoding='utf-8') as f:
                content = f.read()

            # Check if uniform already exists to avoid duplicates
            if uniform_line.strip() in content:
                print(f"  Skipped {vsh_file.name}: uniform already exists")
                continue

            # Split content into lines
            lines = content.split('\n')

            # Add uniform at the beginning
            lines.insert(0, uniform_line)

            # Find the last closing brace and insert before it
            # Search from the end
            last_brace_index = -1
            for i in range(len(lines) - 1, -1, -1):
                if '}' in lines[i]:
                    last_brace_index = i
                    break

            if last_brace_index != -1:
                # Insert before the last closing brace
                lines.insert(last_brace_index, end_line)

                # Write back
                new_content = '\n'.join(lines)
                with open(vsh_file, 'w', encoding='utf-8') as f:
                    f.write(new_content)

                print(f"  ✓ Updated {vsh_file.name}")
                updated_count += 1
            else:
                print(f"  Warning: Could not find closing brace in {vsh_file.name}")

        except Exception as e:
            print(f"  Error processing {vsh_file.name}: {e}")

    print()
    print("=" * 50)
    print(f"Successfully updated {updated_count} .vsh files")

def main():
    # Configuration
    script_dir = Path(__file__).parent.resolve()
    shader_dir = script_dir

    # Define what to inject
    # Line to add at the beginning (uniform declaration)
    uniform_line = "uniform float GameTime;"

    # Line to add before the final closing brace
    # Example: Use GameTime in some way
    end_line = "    // Custom time-based effects can use GameTime here"

    print("Shader Uniform Injector")
    print("=" * 50)
    print(f"Working directory: {shader_dir}")
    print(f"Adding to first line: {uniform_line}")
    print(f"Adding before final }}: {end_line}")
    print()

    # Check if there are any .vsh files
    vsh_files = list(shader_dir.glob('*.vsh'))
    if not vsh_files:
        print(f"Error: No .vsh files found in: {shader_dir}")
        return

    inject_uniform_to_vsh(shader_dir, uniform_line, end_line)

    print("\nComplete!")

if __name__ == "__main__":
    main()